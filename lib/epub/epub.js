...
class Epub {
    ...
    open(input, what) {
        ...
        if (typeof self !== "undefined") {
            inputLocation = self.location.href.origin;
        }
    ...
    unpack(packaging) {
        ...
        if (this.settings.cache && typeof caches != "undefined") {
            ...
            const hrefLocation = typeof window !== "undefined" ? window.location.href : self.location.href;

            if (this.archived) {
                uriComponent = encodeURIComponent(this.locationUrl.toString());
                key = "epubjs-zip/";
                url = new Url(key + uriComponent + path, hrefLocation);
    ...
    findAt(keywords, locationCfi) {
        if (!this.book) {
            return;
        }

        const spineItem = this.book.sections.get(locationCfi);
        globalThis.bkk = this.book;

        let matches = null;

        if (!spineItem) return matches;
        ...
        if (Array.isArray(keywords) && keywords.length > 0) {
            ...
            for (let i = 0, len = keywords.length; i < len; i++) {
                let query = keywords[i];
                try {
                    let match = spineItem.find(query);
    ...
    generateLocations(breakPoint) {
        ...
        return this.locations.generate(this.book.sections, breakPoint, this.load.bind(this)).then(locations => {
