...
class Section {
    constructor(item, hooks, settings) {
        ...
        if (this.href) {
            const firstHttpIndex = this.href.indexOf('http');
            const lastHttpIndex = this.href.lastIndexOf('http');

            if (firstHttpIndex > -1 && firstHttpIndex != lastHttpIndex) {
                let url = decodeURIComponent(this.href.substr(lastHttpIndex));

                if (url) {
                    let pathname = new URL(url).pathname;

                    const filenameIdx = pathname && pathname.lastIndexOf('.epub/');
                    if (filenameIdx > -1) {
                        pathname = pathname.substr(filenameIdx + 5);
                        url = pathname;
                    }
    ...
    load(_request) {
        ...
        if (this.contents) {
            ...
            request(this.url || this.href, type).then(function (xml) {
                this.document = xml;
    ...
    createUrl(request) {
        ...
        return this.render(request).then(text => {
